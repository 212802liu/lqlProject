...
public class BatchInsertOrUpdateUtil {
     ...
     * @return int 影响的总行数
     */
    public <T, U, R> int batchUpdateOrInsert(List<T> data, Class<U> mapperClass, BiFunction<List<T>, U, R> function) throws Exception {
        if (data.size() == 0) {
            log.warn("批插入的数据为空！");
            return 0;
        }
        ...
        try {
            ...
            for (int index = 0; index < size; ) {
                ...
                if (batchLastIndex >= size) {
                    ...
                    log.info("batchUpdateOrInsert", "index:" + index + " batchLastIndex:" + batchLastIndex);
                    // 数据插入完毕，退出循环
                    break;
                } else {
                    // 多插几次 BATCH_COUNT
            ...
            log.error("batchUpdateOrInsert 方法，批处理失败！",e);
